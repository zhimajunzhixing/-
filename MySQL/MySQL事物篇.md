---
title: MySQL事物篇
date: 2023-03-09T12:20:20Z
lastmod: 2023-03-18T20:27:36Z
---
...
![image-20220708124306444](assets/image-20220708124306444-20230309124253-1e7itcz.png)
...
![image-20220708164610193](assets/image-20220708164610193-20230309124253-kv4gfr5.png)
...
* **提交的（committed）**
  ...
  ![MySQL事物篇.assets/image-20220708171859055.png](assets/image-20220708171859055-20230309124253-mcvvjgn.png)
...
![MySQL事物篇.assets/image-20220708201221316.png](assets/image-20220708201221316-20230309124253-2fhz3j8.png)
...
![image-20220708214453902](assets/image-20220708214453902-20230309124253-bkdokat.png)
...
![image-20220708215109480](assets/image-20220708215109480-20230309124253-69wbook.png)
...
![image-20220708215626435](assets/image-20220708215626435-20230309124253-fs6v241.png)
...
![image-20220708220102342](assets/image-20220708220102342-20230309124253-zcvl13n.png)
...
![MySQL事物篇.assets/image-20220708220228436.png](assets/image-20220708220228436-20230309124253-5i2ndwx.png)
...
![image-20220708220917267](assets/image-20220708220917267-20230309124253-s9rqopl.png)

`脏写 ​`怎么没涉及到？因为脏写这个问题太严重了，不论是哪种隔离级别，都不允许脏写的情况发生。

不同的隔离级别有不同的现象，并有不同的锁和并发机制，隔离级别越高，数据库的并发性能就越差，4 种事务隔离级别与并发性能的关系如下：

![MySQL事物篇.assets/image-20220708220957108.png](assets/image-20220708220957108-20230309124253-bii42lm.png)

### 3.4 MySQL支持的四种隔离级别

![MySQL事物篇.assets/image-20220708221639979.png](assets/image-20220708221639979-20230309124253-m7rjts6.png)
...
![MySQL事物篇.assets/image-20220708223250773.png](assets/image-20220708223250773-20230309124253-0irtfab.png)

**演示1. 读未提交之脏读**

设置隔离级别为未提交读：

![image-20220710193920008](assets/image-20220710193920008-20230309124253-eh1pr4k.png)

脏读就是指当前事务就在访问数据，并且对数据进行了修改，而这种修改还没有提交到数据库中，这时，另外一个事务也访问了这个数据，然后使用了这个数据。

**演示2：读已提交**

![image-20220710194440101](assets/image-20220710194440101-20230309124253-w007v18.png)

**演示3. 不可重复读**

设置隔离级别为可重复读，事务的执行流程如下：

![image-20220710194144826](assets/image-20220710194144826-20230309124253-lbqd4ji.png)

当我们将当前会话的隔离级别设置为可重复读的时候，当前会话可以重复读，就是每次读取的结果集都相同，而不管其他事务有没有提交。但是在可重复读的隔离级别上会出现幻读的问题。

**演示4：幻读**

![image-20220710194042096](assets/image-20220710194042096-20230309124253-7keretm.png)

![MySQL事物篇.assets/image-20220710194612317.png](assets/image-20220710194612317-20230309124253-zpdkrpf.png)
...
![image-20220710202517977](assets/image-20220710202517977-20230309124253-b3vh78d.png)
...
![image-20220710204114543](assets/image-20220710204114543-20230309124253-wbbn4mv.png)
...
![image-20220710204427616](assets/image-20220710204427616-20230309124253-jc8pxb8.png)

### 1.4 redo的整体流程

以一个更新事务为例，redo log 流转过程，如下图所示：

![image-20220710204810264](assets/image-20220710204810264-16574572910841-20230309124253-xrl1p7k.png)
...
![image-20220710205015302](assets/image-20220710205015302-20230309124253-oj9oycz.png)
...
![MySQL事物篇.assets/image-20220710205948156.png](assets/image-20220710205948156-20230309124253-s6h70ve.png)

另外，InnoDB存储引擎有一个后台线程，每隔`1秒`，就会把`redo log buffer`中的内容写到文件系统缓存(`page cache`)，然后调用刷盘操作。

![image-20220710210339724](assets/image-20220710210339724-20230309124253-3ocyg9w.png)

也就是说，一个没有提交事务的`redo log`记录，也可能会刷盘。因为在事务执行过程 redo log 记录是会写入 `redo log buffer`中，这些redo log 记录会被`后台线程`刷盘。

![image-20220710210532805](assets/image-20220710210532805-20230309124253-up0c3xl.png)
...
![MySQL事物篇.assets/image-20220710210751414.png](assets/image-20220710210751414-20230309124253-jg7qqmm.png)

![MySQL事物篇.assets/image-20220710211318120.png](assets/image-20220710211318120-20230309124253-vgajbkm.png)

![MySQL事物篇.assets/image-20220710211335379.png](assets/image-20220710211335379-20230309124253-0ldlzfm.png)

![MySQL事物篇.assets/image-20220710211618789.png](assets/image-20220710211618789-20230309124253-84riztn.png)

![MySQL事物篇.assets/image-20220710211831675.png](assets/image-20220710211831675-20230309124253-eyptjxv.png)

![MySQL事物篇.assets/image-20220710212041563.png](assets/image-20220710212041563-20230309124253-9wejbp2.png)
...
![MySQL事物篇.assets/image-20220710215001482.png](assets/image-20220710215001482-20230309124253-rzk60d9.png)
...
![MySQL事物篇.assets/image-20220710215353893.png](assets/image-20220710215353893-20230309124253-e81fj3k.png)
...
![image-20220710220653131](assets/image-20220710220653131-20230309124253-tetrmv7.png)

#### 2. redo 日志写入log buffer

![MySQL事物篇.assets/image-20220710220838744.png](assets/image-20220710220838744-20230309124253-oq6tiee.png)

![image-20220710220919271](assets/image-20220710220919271-20230309124253-gxon7ce.png)

![MySQL事物篇.assets/image-20220710221221981.png](assets/image-20220710221221981-20230309124253-sdzvwmx.png)

![image-20220710221318271](assets/image-20220710221318271-20230309124253-v72sjio.png)
...
![image-20220710221620291](assets/image-20220710221620291-20230309124253-5j4z00e.png)
...
![MySQL事物篇.assets/image-20220710223117420.png](assets/image-20220710223117420-20230309124253-3cb6hcz.png)

![image-20220710223135374](assets/image-20220710223135374-20230309124253-lrh8vw1.png)

真正的redo日志都是存储到占用`496`字节大小的`log block body`中，图中的`log block header`和`log block trailer`存储的是一些管理信息。我们来看看这些所谓`管理信息`都有什么。

![image-20220711144546439](assets/image-20220711144546439-20230309124253-617e0pg.png)

![MySQL事物篇.assets/image-20220711144608223.png](assets/image-20220711144608223-20230309124253-9cxornb.png)
...
![MySQL事物篇.assets/image-20220711152137012.png](assets/image-20220711152137012-20230309124253-c5nkste.png)

![image-20220711152242300](assets/image-20220711152242300-20230309124253-5qygj3z.png)
...
![MySQL事物篇.assets/image-20220711152631108.png](assets/image-20220711152631108-20230309124253-8mfra43.png)

如果 write pos 追上 checkpoint ，表示`日志文件组`满了，这时候不能再写入新的 redo log记录，MySQL 得 停下来，清空一些记录，把 checkpoint 推进一下。

![MySQL事物篇.assets/image-20220711152802294.png](assets/image-20220711152802294-20230309124253-le3jnt2.png)

### 1.9 redo log 小结

![MySQL事物篇.assets/image-20220711152930911.png](assets/image-20220711152930911-20230309124253-g5tl89f.png)
...
![MySQL事物篇.assets/image-20220711153523704.png](assets/image-20220711153523704-20230309124253-pmxmuoi.png)

### 2.2 Undo日志的作用

* **作用1：回滚数据**

![MySQL事物篇.assets/image-20220711153645204.png](assets/image-20220711153645204-20230309124253-1kd76do.png)
...
![MySQL事物篇.assets/image-20220711154936382.png](assets/image-20220711154936382-20230309124253-7kdje5r.png)

![MySQL事物篇.assets/image-20220711155044078.png](assets/image-20220711155044078-20230309124253-e6odezs.png)
...
![MySQL事物篇.assets/image-20220711162414928.png](assets/image-20220711162414928-20230309124253-o7ko3q6.png)

**只有Buffer Pool的流程：**

![image-20220711162505008](assets/image-20220711162505008-20230309124253-jh2fs08.png)

**有了Redo Log和Undo Log之后：**

![image-20220711162642305](assets/image-20220711162642305-20230309124253-awj235v.png)

在更新Buffer Pool中的数据之前，我们需要先将该数据事务开始之前的状态写入Undo Log中。假设更新到一半出错了，我们就可以通过Undo Log来回滚到事务开始前。

#### 2. 详细生成过程

![MySQL事物篇.assets/image-20220711162919157.png](assets/image-20220711162919157-20230309124253-dzmjtiy.png)
...
![image-20220711163725129](assets/image-20220711163725129-20230309124253-budj52x.png)
...
![image-20220711164138414](assets/image-20220711164138414-20230309124253-jwpjyw7.png)
...
![image-20220711164421494](assets/image-20220711164421494-20230309124253-cjll7ds.png)
...
![image-20220711165612956](assets/image-20220711165612956-20230309124253-pesb1tb.png)
...
![MySQL事物篇.assets/image-20220711165954976.png](assets/image-20220711165954976-20230309124253-m4bc0e7.png)
...
![image-20220711181120639](assets/image-20220711181120639-20230309124253-p3fv1qe.png)

当一个事务想对这条记录做改动时，首先会看看内存中有没有与这条记录关联的 `锁结构` ，当没有的时候 就会在内存中生成一个 `锁结构` 与之关联。比如，事务`​ T1` 要对这条记录做改动，就需要生成一个 `锁结构` 与之关联：

![image-20220711192633239](assets/image-20220711192633239-20230309124253-bjx79rj.png)
...
![image-20220711193732567](assets/image-20220711193732567-20230309124253-qir14rk.png)
...
![image-20220711194904328](assets/image-20220711194904328-20230309124253-24wjwpm.png)
...
![MySQL事物篇.assets/image-20220711202206405.png](assets/image-20220711202206405-20230309124253-1mhkgy5.png)
...
![MySQL事物篇.assets/image-20220711203250284.png](assets/image-20220711203250284-20230309124253-e2312gi.png)
...
![image-20220711203519162](assets/image-20220711203519162-20230309124253-jacw83y.png)

### 3.1 从数据操作的类型划分：读锁、写锁

![MySQL事物篇.assets/image-20220711203723941.png](assets/image-20220711203723941-20230309124253-z5dhj3v.png)
...
![MySQL事物篇.assets/image-20220711204843684.png](assets/image-20220711204843684-20230309124253-aafszyu.png)

#### 1. 锁定读

![MySQL事物篇.assets/image-20220711212931912.png](assets/image-20220711212931912-20230309124253-rfgnmao.png)

![MySQL事物篇.assets/image-20220711213741630.png](assets/image-20220711213741630-20230309124253-qqky4cp.png)

![MySQL事物篇.assets/image-20220711214013208.png](assets/image-20220711214013208-20230309124253-1vnione.png)

#### 2. 写操作

![MySQL事物篇.assets/image-20220711214412163.png](assets/image-20220711214412163-20230309124253-f10svb5.png)

### 3.2 从数据操作的粒度划分：表级锁、页级锁、行锁

![MySQL事物篇.assets/image-20220711214719510.png](assets/image-20220711214719510-20230309124253-8jfo5yb.png)

#### 1. 表锁（Table Lock）

![MySQL事物篇.assets/image-20220711214805088.png](assets/image-20220711214805088-20230309124253-lupabi9.png)
...
![MySQL事物篇.assets/image-20220711220342251.png](assets/image-20220711220342251-20230309124253-scgtvyv.png)

或者

![MySQL事物篇.assets/image-20220711220418859.png](assets/image-20220711220418859-20230309124253-luqrqlh.png)
...
![MySQL事物篇.assets/image-20220711220442269.png](assets/image-20220711220442269-20230309124253-jiqqiyn.png)
...
![MySQL事物篇.assets/image-20220711220502141.png](assets/image-20220711220502141-20230309124253-jbo10ga.png)

步骤5：加读锁

我们为mylock表加read锁（读阻塞写），观察阻塞的情况，流程如下：

![image-20220711220553225](assets/image-20220711220553225-20230309124253-8en3o9n.png)

![image-20220711220616537](assets/image-20220711220616537-20230309124253-1xydysu.png)

步骤6：加写锁

为mylock表加write锁，观察阻塞的情况，流程如下：

![image-20220711220711630](assets/image-20220711220711630-20230309124253-eihcx1n.png)

![image-20220711220730112](assets/image-20220711220730112-20230309124253-mq7yn05.png)
...
* 表独占写锁（Table Write Lock）
  ...
  ![image-20220711220929248](assets/image-20220711220929248-20230309124253-mxqxduv.png)
...
![MySQL事物篇.assets/image-20220711222132300.png](assets/image-20220711222132300-20230309124253-fci8glm.png)
...
![MySQL事物篇.assets/image-20220712124209006.png](assets/image-20220712124209006-20230309124253-dds3e00.png)
...
![MySQL事物篇.assets/image-20220712175552985.png](assets/image-20220712175552985-20230309124253-zt3j501.png)
...
![image-20220713142808924](assets/image-20220713142808924-20230309124253-m5nu069.png)

通过会话C可以看出会话B被阻塞，这是由于会话A拿到了teacher表的元数据读锁，会话B想申请teacher表的元数据写锁，由于读写锁互斥，会话B需要等待会话A释放元数据锁才能执行。

![MySQL事物篇.assets/image-20220713143156759.png](assets/image-20220713143156759-20230309124253-fpnsdm1.png)
...
![MySQL事物篇.assets/image-20220713161549241.png](assets/image-20220713161549241-20230309124253-ikytxc7.png)

student表中的聚簇索引的简图如下所示。

![image-20220713163353648](assets/image-20220713163353648-20230309124253-b8yidco.png)
...
![image-20220713164811567](assets/image-20220713164811567-20230309124253-g3aqzpq.png)

举例如下：

![image-20220713164948405](assets/image-20220713164948405-20230309124253-woacgg4.png)
...
![image-20220713171650888](assets/image-20220713171650888-20230309124253-6splh1e.png)
...
![MySQL事物篇.assets/image-20220713174726264.png](assets/image-20220713174726264-20230309124253-9r7zgk6.png)
...
![image-20220713174108634](assets/image-20220713174108634-20230309124253-7rybap4.png)
...
![image-20220713174551814](assets/image-20220713174551814-20230309124253-9t5k3jg.png)

![image-20220713174602102](assets/image-20220713174602102-20230309124253-nxecixj.png)

![MySQL事物篇.assets/image-20220713175032619.png](assets/image-20220713175032619-20230309124253-tj87qgy.png)

![MySQL事物篇.assets/image-20220713192418730.png](assets/image-20220713192418730-20230309124253-vhg2fif.png)
...
![image-20220713192549340](assets/image-20220713192549340-20230309124253-8nsgh4a.png)
...
![MySQL事物篇.assets/image-20220713203124889.png](assets/image-20220713203124889-20230309124253-bfv0hxz.png)

![MySQL事物篇.assets/image-20220713203532124.png](assets/image-20220713203532124-20230309124253-letei7g.png)

![MySQL事物篇.assets/image-20220713203619704.png](assets/image-20220713203619704-20230309124253-3aeijl7.png)

![MySQL事物篇.assets/image-20220713203714577.png](assets/image-20220713203714577-20230309124253-fvyu35k.png)
...
![MySQL事物篇.assets/image-20220713204544767.png](assets/image-20220713204544767-20230309124253-h6zxn7v.png)

![MySQL事物篇.assets/image-20220713205010502.png](assets/image-20220713205010502-20230309124253-awsvuil.png)

![MySQL事物篇.assets/image-20220713205135694.png](assets/image-20220713205135694-20230309124253-apkb4wr.png)
...
![MySQL事物篇.assets/image-20220713210951100.png](assets/image-20220713210951100-20230309124253-edmpq4j.png)

![MySQL事物篇.assets/image-20220713211139670.png](assets/image-20220713211139670-20230309124253-tvre7w5.png)
...
![MySQL事物篇.assets/image-20220713211417909.png](assets/image-20220713211417909-20230309124253-8dw6gc0.png)

### 3.4 按加锁的方式划分：显式锁、隐式锁

#### 1. 隐式锁

![MySQL事物篇.assets/image-20220713211525845.png](assets/image-20220713211525845-20230309124253-shg48cs.png)
...
![MySQL事物篇.assets/image-20220713214522709.png](assets/image-20220713214522709-20230309124253-99xzyp3.png)
...
![image-20220713220714098](assets/image-20220713220714098-20230309124253-p5fwowj.png)

**举例2：**

用户A给用户B转账100，再次同时，用户B也给用户A转账100。这个过程，可能导致死锁。

![MySQL事物篇.assets/image-20220713220936236.png](assets/image-20220713220936236-20230309124253-91ld9jz.png)
...
![MySQL事物篇.assets/image-20220713221418100.png](assets/image-20220713221418100-20230309124253-1uzec13.png)
...
![image-20220713221758941](assets/image-20220713221758941-20230309124253-4sr6avs.png)

基于这两个信息，可以绘制wait-for graph（等待图）

![image-20220713221830455](assets/image-20220713221830455-20230309124253-98e5flp.png)
...
![MySQL事物篇.assets/image-20220714131008260.png](assets/image-20220714131008260-20230309124253-zkcxy98.png)

## 4. 锁的内部结构

我们前边说对一条记录加锁的本质就是在内存中创建一个`锁结构`​与之关联，那么是不是一个事务对多条记录加锁，就要创建多个`锁`​[结构]()``​呢？比如：
...
![image-20220714132306208](assets/image-20220714132306208-20230309124253-lvqfv6r.png)
...
![image-20220714133319666](assets/image-20220714133319666-20230309124253-a16hioq.png)
...
![image-20220714134603698](assets/image-20220714134603698-20230309124253-hjk0sjo.png)
...
![image-20220714134623052](assets/image-20220714134623052-16577775838551-20230309124253-sztqhvs.png)
...
![image-20220714134742049](assets/image-20220714134742049-20230309124253-8brhbww.png)
...
![image-20220714134822160](assets/image-20220714134822160-20230309124253-o3zui9j.png)
...
![image-20220714134846740](assets/image-20220714134846740-20230309124253-05qmuci.png)
...
![image-20220714134923414](assets/image-20220714134923414-20230309124253-ukogrih.png)
...
![image-20220714134945012](assets/image-20220714134945012-20230309124253-sad8z23.png)
...
![image-20220714135007118](assets/image-20220714135007118-20230309124253-5fg7ff3.png)
...
![image-20220714135025045](assets/image-20220714135025045-16577778257713-20230309124253-pzw2io4.png)
...
![image-20220714135047760](assets/image-20220714135047760-20230309124253-kqjjo3j.png)
...
![image-20220714135130668](assets/image-20220714135130668-20230309124253-i5r6qrp.png)
...
![image-20220714135206504](assets/image-20220714135206504-20230309124253-f235979.png)
...
![image-20220714135300189](assets/image-20220714135300189-20230309124253-82melzz.png)
...
![image-20220714135333089](assets/image-20220714135333089-20230309124253-v3ypzhu.png)
...
![image-20220714140441064](assets/image-20220714140441064-20230309124253-tqioxh6.png)

![MySQL事物篇.assets/image-20220714140510426.png](assets/image-20220714140510426-20230309124253-67i8f2g.png)

![image-20220714140541555](assets/image-20220714140541555-20230309124253-83cdovb.png)
...
![MySQL事物篇.assets/image-20220714140716427.png](assets/image-20220714140716427-20230309124253-6bpzb73.png)

假设插入该记录的`事务id`为`8`，那么此刻该条记录的示意图如下所示：

![image-20220714140801595](assets/image-20220714140801595-20230309124253-08q7w5x.png)

> insert undo只在事务回滚时起作用，当事务提交后，该类型的undo日志就没用了，它占用的Undo Log Segment也会被系统回收（也就是该undo日志占用的Undo页面链表要么被重用，要么被释放）。

假设之后两个事务id分别为 `10` 、 `20` 的事务对这条记录进行`​ UPDATE` 操作，操作流程如下：

![image-20220714140846658](assets/image-20220714140846658-20230309124253-lo5w30p.png)

![MySQL事物篇.assets/image-20220714140908661.png](assets/image-20220714140908661-20230309124253-y4dn69h.png)

每次对记录进行改动，都会记录一条undo日志，每条undo日志也都有一个 `roll_pointer` 属性 （ `INSERT` 操作对应的undo日志没有该属性，因为该记录并没有更早的版本），可以将这些 `undo日志` 都连起来，串成一个链表：

![image-20220714141012874](assets/image-20220714141012874-20230309124253-7k9bnat.png)
...
![MySQL事物篇.assets/image-20220714141154235.png](assets/image-20220714141154235-20230309124253-b42w07u.png)
...
![MySQL事物篇.assets/image-20220714142254768.png](assets/image-20220714142254768-20230309124253-8hxg5m0.png)
...
![MySQL事物篇.assets/image-20220715130639408.png](assets/image-20220715130639408-20230309124253-8yzewvf.png)

在隔离级别为读已提交（Read Committed）时，一个事务中的每一次 SELECT 查询都会重新获取一次 Read View。

如表所示：

![image-20220715130843147](assets/image-20220715130843147-20230309124253-r7qg1s0.png)

> 注意，此时同样的查询语句都会重新获取一次 Read View，这时如果 Read View 不同，就可能产生不可重复读或者幻读的情况。

当隔离级别为可重复读的时候，就避免了不可重复读，这是因为一个事务只在第一次 SELECT 的时候会获取一次 Read View，而后面所有的 SELECT 都会复用这个 Read View，如下表所示：

![image-20220715130916437](assets/image-20220715130916437-20230309124253-zet5als.png)

## 5. 举例说明

![MySQL事物篇.assets/image-20220715131200077.png](assets/image-20220715131200077-20230309124253-ws6bhtt.png)
...
![image-20220715133640655](assets/image-20220715133640655-20230309124253-plroqku.png)
...
![MySQL事物篇.assets/image-20220715134540737.png](assets/image-20220715134540737-20230309124253-v6j014k.png)
...
![image-20220715134839081](assets/image-20220715134839081-20230309124253-g034t9v.png)
...
![MySQL事物篇.assets/image-20220715135017000.png](assets/image-20220715135017000-20230309124253-jcnic60.png)

![MySQL事物篇.assets/image-20220715135143939.png](assets/image-20220715135143939-20230309124253-b3zmd8b.png)
...
![image-20220715140006061](assets/image-20220715140006061-20230309124253-51p8x2j.png)
...
![MySQL事物篇.assets/image-20220715140155744.png](assets/image-20220715140155744-20230309124253-qqxubty.png)
...
![image-20220715140354217](assets/image-20220715140354217-20230309124253-v265ah5.png)
...
![MySQL事物篇.assets/image-20220715140555172.png](assets/image-20220715140555172-20230309124253-ao7zd19.png)

![MySQL事物篇.assets/image-20220715140620328.png](assets/image-20220715140620328-20230309124253-f459zrj.png)
...
![MySQL事物篇.assets/image-20220715141002035.png](assets/image-20220715141002035-20230309124253-hksydm0.png)
...
![image-20220715141208667](assets/image-20220715141208667-20230309124253-mygpav9.png)
...
![image-20220715141243993](assets/image-20220715141243993-20230309124253-7fuzwdb.png)
...
![MySQL事物篇.assets/image-20220715141413135.png](assets/image-20220715141413135-20230309124253-uutmc8w.png)

通过MVCC我们可以解决：

![MySQL事物篇.assets/image-20220715141515370.png](assets/image-20220715141515370-20230309124253-4kfm5js.png)
